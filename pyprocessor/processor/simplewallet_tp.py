...
class SimpleWalletTransactionHandler(TransactionHandler):
    ...
    def apply(self, transaction, context):
        ...
        if operation == "deposit":
            self._make_deposit(context, deviceID, from_key)
        elif operation == "withdraw":
            self._make_withdraw(context, deviceID, from_key)
        elif operation == "add":
            productID = payload_list[2]
            senderAcc = payload_list[3]
            courierAcc = payload_list[4]
            receiverAcc = payload_list[5]
            PUlat = payload_list[6]
            PUlong = payload_list[7]
            maxTemp = payload_list[8]
            minTemp = payload_list[9]
            maxHumi = payload_list[10]
            minHumi = payload_list[11]
            delilat = payload_list[12]
            delilong = payload_list[13]
            deliFunds = payload_list[14]
            self._make_add(context, deviceID, productID, senderAcc, courierAcc, receiverAcc, PUlat, PUlong,maxTemp, minTemp, maxHumi, minHumi, delilat, delilong, deliFunds, from_key)
        elif operation == "transfer":
            if len(payload_list) == 3:
                to_key = payload_list[2]
            self._make_transfer(context, deviceID, to_key, from_key)
        else:
    ...
    def _make_add(self, context, deviceID, productID, senderAcc, courierAcc, receiverAcc, PUlat, PUlong,maxTemp, minTemp, maxHumi, minHumi, delilat, delilong, deliFunds, from_key):
        wallet_address = self._get_wallet_address(from_key)
        LOGGER.info('Got the key {} and the wallet address {} '.format(
            from_key, wallet_address))
        current_entry = context.get_state([wallet_address])
        new_transaction = current_entry

        if current_entry == []:
            LOGGER.info('No previous deposits, creating new deposit {} '
                        .format(from_key))
            new_transaction = str('\nDevice ID: '+deviceID+'\nProduct ID: '+productID+ '\nSender Account: '+senderAcc+  '\nCourier Account: '+courierAcc+ '\nReceiver Account: '+receiverAcc
                                  + '\nPick-up Lattitude: '+PUlat+ '\nPick-up Longitude: '+PUlong+ '\nMaximum Temperature: '+maxTemp+ '\nMinumum Temperature: '+minTemp
                                  + '\nMaximum Humidity: '+maxHumi+ '\nMinumum Humidity: '+minHumi+ '\nDelivery Lattitude: '+delilat+ '\nDelivery Longitude: '+delilong+ '\nDelivery Funds: '+deliFunds)
        else:
            LOGGER.info('Transaction information not new, {} '
                        .format(from_key))

        state_data = str(new_transaction).encode('utf-8')
        addresses = context.set_state({wallet_address: state_data})

        if len(addresses) < 1:
            raise InternalError("State Error")

    def _make_transaction(self, context, amount, from_key):
        wallet_address = self._get_wallet_address(from_key)
        LOGGER.info('Got the key {} and the wallet address {} '.format(
            from_key, wallet_address))
        current_entry = context.get_state([wallet_address])
