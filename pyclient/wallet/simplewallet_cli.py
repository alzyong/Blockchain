...
def add_add_parser(subparsers, parent_parser):
    ...
    parser = subparsers.add_parser(
        'add',
        help='add transaction to account',
        parents=[parent_parser])

    parser.add_argument(
        'deviceID',
        type=str,
        help='ID of device associated with transaction.')
...
def do_add(args):
    ...
    keyfile = _get_keyfile(args.transactionID)

    client = SimpleWalletClient(baseUrl=DEFAULT_URL, keyFile=keyfile)

    response = client.add(args.deviceID,args.productID,args.senderAcc, args.courierAcc,args.receiverAcc,args.PUlat, args.PUlong,
                          args.maxTemp, args.minTemp, args.maxHumi, args.minHumi, args.delilat, args.delilong,args.deliFunds)
